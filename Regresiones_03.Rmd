...
## 4. Regresión Lineal Múltiple con Interacciones
...
nombres <- names(data_reg[-6])
nombres <- c(nombres,"medv_predict_mi3vars")
names(data_reg) <-nombres 
rm(nombres)
...
$$
medv = 8.16695*lstat + 40.27241*rm + 11.53867*ptratio - 204.61419chas - 1.25939*lstat*rm
$$

$$
-0.35211*lstat*ptratio -1.74752*rm*ptratio+13.90197*ptratio*chas
$$

$$
+0.04283*lstat*rm*ptratio-1.55780rm*ptratio*chas-233.75030 
$$
...
nombres <- names(data_reg[-10])
nombres <- c(nombres,"medv_predict_mi4vars")
names(data_reg) <-nombres 
rm(nombres)
...
El RMSE del modelo con interacción de 2 variables se calcula en 4.18, el del modelo con interacción de 3 variables se observa en 3.99 mientras que el de 4 variables alcanza un valor de 3.46. Las diferencias del primer y segundo modelo en relación con el tercero empieza a ser significante pues son de 500 dólares y 700 dólares respectivamente.
...
RMSE_3[,1] <- c("01_RLS_lstat","02_RLS_rm","03_RLM_12vars","04_RLM_10vars"
                ,"05_RLM_2vars","06_RLMI_2vars", "07_RLMI_3vars", "08_RLMI_4vars")
```
